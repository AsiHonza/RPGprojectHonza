...
# 1. State hook for gameMode
if "const [gameMode, setGameMode]" not in content:
    content = content.replace(
        'const [dndClass, setDndClass] = useState("Bojovník");',
        'const [dndClass, setDndClass] = useState("Bojovník");\n  const [gameMode, setGameMode] = useState("sandbox");'
    )

# 2. Add to fetch request in startNewGame
if 'game_mode: gameMode' not in content:
    content = content.replace(
...
ui_toggle = """              <div className="bg-[#e3dcc8] p-4 rounded border border-[#90a4ae] flex flex-col gap-2">
