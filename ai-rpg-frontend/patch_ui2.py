...
# I will use a regex to replace the entire <div className="space-y-4"> block inside gameState === "menu"
pattern = re.compile(r'<div className="space-y-4">.*?Založit novou postavu\s*</button>\s*</div>', re.DOTALL | re.IGNORECASE)
...
# Fallback regex if encoding broke the word 'Založit'
pattern_fallback = re.compile(r'<div className="space-y-4">.*Zalo.{1,3}it novou postavu\s*</button>\s*</div>', re.DOTALL | re.IGNORECASE)

if pattern_fallback.search(content):
    content = pattern_fallback.sub(new_menu_ui, content)
    with codecs.open('src/app/page.tsx', 'w', 'utf-8') as f:
        f.write(content)
    print("UI patched via regex")
else:
