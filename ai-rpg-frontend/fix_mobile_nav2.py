...
# Replace the entire `<div className="flex gap-2 text-[#2b4c5e]">` up to the closing `</div>` before `{/* XP Bar */}`
start_idx = content.find('<div className="flex gap-2 text-[#2b4c5e]">')
end_idx = content.find('</div>\n        </div>\n        \n        {/* XP Bar */}')

if start_idx != -1 and end_idx != -1:
    end_idx += len('</div>')
    
    new_ui = """<div className="flex flex-col sm:flex-row items-end sm:items-center gap-2 sm:gap-4 text-[#2b4c5e]">
            {/* Status (HP & Food) */}
