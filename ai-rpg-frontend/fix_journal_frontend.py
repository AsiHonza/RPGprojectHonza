...
# 1. State for Journal
if "const [journal, setJournal]" not in content:
    content = content.replace(
        'const [quests, setQuests] = useState<{nazev: string, popis: string, stav: string}[]>([]);',
        'const [quests, setQuests] = useState<{nazev: string, popis: string, stav: string}[]>([]);\n  const [journal, setJournal] = useState<string[]>([]);\n  const [journalOpen, setJournalOpen] = useState(false);'
    )

# 2. Add to loadGame and startNewGame
content = content.replace(
...
# 3. Handle action response
content = content.replace(
    'if (data.pointsOfInterest) setPointsOfInterest(data.pointsOfInterest);',
    'if (data.pointsOfInterest) setPointsOfInterest(data.pointsOfInterest);\n        if (data.state && data.state.journal) setJournal(data.state.journal);'
)

# 4. Add the journal button
journal_button = """              <button onClick={() => setJournalOpen(true)} className="flex items-center gap-1 font-bold text-[#2b4c5e] hover:text-[#b74b4b] transition cursor-pointer bg-[#e3dcc8] px-2 py-1 rounded border border-[#90a4ae] relative" title="Příběhový deník (Příběh a Shrnutí)">
                <ScrollText size={18} />
              </button>"""

if "Příběhový deník" not in content:
    content = content.replace(
...
if "Journal Modal" not in content:
    content = content.replace(
        '{/* Skills Modal */}',
        journal_modal + '\n      {/* Skills Modal */}'
    )
