const fs = require("fs");
const path = "src/features/combat/combatEngine.ts";
...
let enemyAttackLogic = \    // Execute Intent
    if (enemy.intent === "attack" || enemy.intent === "heavy_attack") {
      ...
      if (RACES[playerRace]?.trait.id === "gnome_cunning" && dmg > 5 && rollDie(100) <= 25) {
        dmg = 0;
        logEntries.push(\\\✨ Technomagický štít tě ochránil před mocným útokem od **\**!\\\);
      }
      ...
      currentHp -= dmg;
      logEntries.push(\\\⚔️ **\** zaútočil a udělil ti **\ poškození**!\\\);
      
      // Tiefling Hellish Rebuke (Thorns)
      if (RACES[playerRace]?.trait.id === "hellish_rebuke" && dmg > 0) {
        enemy.hp -= 2;
        logEntries.push(\\\🔥 Pekelná odplata! **\** utržil 2 poškození z tvé krve.\\\);
      }
      
    } else if (enemy.intent === "defend") {\;

code = code.replace(
  /    \/\/ Execute Intent[\\s\\S]*?} else if \\(enemy\\.intent === "defend"\\) {/,
  enemyAttackLogic
);
