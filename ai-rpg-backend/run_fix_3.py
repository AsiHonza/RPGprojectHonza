...
start = content.find('client = genai.Client(api_key=os.environ.get("GEMINI_API_KEY"))')
end = content.find("response = client.models.generate_content(", start)

if start != -1 and end != -1:
    new_prompt = """client = genai.Client(api_key=os.environ.get("GEMINI_API_KEY"))
        world_context = ""
        if world_data:
            import json
